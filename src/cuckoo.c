...
/*
 * if no locking is used at all
 */
#if defined(NO_LOCKING) 
static void fg_lock(uint32_t i1, uint32_t i2) {}
static void fg_unlock(uint32_t i1, uint32_t i2) {}
#endif 

/*
 * this is for the case we use fine-grained bucket locking
 * item locks are striped, in total we use 8192 buckets
 */
...
#define IS_SLOT_EMPTY(i,j) (buckets[i].vals[j] == 0)
 ...
 */
static item *try_read(const char *key, const size_t nkey, TagType tag, size_t i) {
...
/* #else // PAR_CUCKOO_WIDTH == 1 */
/* /\*  */
/*  * Make bucket i1 availalbe to insert a new item, */
/*  * return the empty slot  on success,  -1  otherwise */
/*  * */
/*  * @param  i1:    the bucket index */
/*  * @param  depth: the current cuckoo depth */
/*  *\/ */
/* static int cuckoo(size_t i1, int depth) { */
/*     if (depth > MAX_CUCKOO_COUNT) { */
/*         printf("%u max cuckoo achieved, abort\n", MAX_CUCKOO_COUNT); */
/*         return -1; */
/*     } */
...
/*      *\/ */
/*     for (size_t j = 0; j < bucket_size; j ++) */
/*         if (IS_SLOT_EMPTY(i1, j)) */
/*             return j; */

/*     /\* */
/*      * No space, let's kick some victim item out  */
/*      *\/ */
/*     size_t j1 = rand() % bucket_size; // pick the victim item */
/*     for (size_t k = 0; k < bucket_size; k ++) { */
/*         j1            = (j1 + 1) % bucket_size; */
/*         ValueType val = buckets[i1].vals[j1]; */
/*         TagType   tag = buckets[i1].tags[j1]; */
/*         size_t    i2  = _alt_index(i1, tag); */
/*         size_t lock   = _lock_index(i1, i2, tag); */

/*         /\* */
/*          * Try to make bucket i2 available */
/*          *\/ */
/*         int j2 = cuckoo(i2, depth + 1); */
/*         if (j2 < 0) */
/*             return -1; */
...
/*          * If that happened, just... try again. */
/*          *\/ */
/*         if (buckets[i1].tags[j1] != tag) { */
/*             /\* try again *\/ */
/*             return cuckoo(i1,  depth + 1); */
/*         } */
...
/*         buckets[i2].tags[j2] = tag; //buckets[i1].tags[j1]; */
/*         buckets[i2].vals[j2] = val; //buckets[i1].vals[j1]; */
